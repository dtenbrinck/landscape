...
% Last Modified by GUIDE v2.5 31-Aug-2016 19:23:20
...
h.subax1.Parent = hObject;
h.subax2 = axes('Visible','off');
h.subax2.Parent = hObject;
h.subax3 = axes('Visible','off'); 
h.subax3.Parent = hObject;
...
set(h.tbColorbar,'Parent',h.paVis_Pro,...
    'Units','normalized',...
...
set(h.paVis_Slice,'Parent',h.paControls,...
    'Units','normalized',...
    'Position',get(h.paVisPH,'Position'));
set(h.txSlicer,'Parent',h.paVis_Slice,...
    'String',['Slice ', num2str(1),' of ', num2str(get(h.slSlices,'Max'))]);
set(h.txSpeed,'Parent',h.paVis_Slice);
...
set(h.cbLabelAxis_Slice,'Parent',h.paVis_Slice,...
    ...
    'FontUnits','normalized',...
    'Position',get(h.cbLAPH,'Position'));
set(h.tbColorbar_Slice,'Parent',h.paVis_Slice,...
    'Units','normalized',...
    'Position',get(h.tbVisTop,'Position'));
...
function labelAxis(h,datasize,cT,color)
% cT: currentTab
if nargin<4
    color = 'magenta';
end
if strcmp(cT,'Pro')||strcmp(cT,'Slice')
    text(h.axMain,2, datasize(2)/2,max(h.proMat(:))*2/3,'Tail','FontSize',12,'Color',color,'Rotation',90);
    text(h.axMain,datasize(1)-1, datasize(2)/2,max(h.proMat(:))*2/3,'Head','FontSize',12,'Color',color,'Rotation',90);
    text(h.axMain,datasize(1)/2,2,max(h.proMat(:))*2/3,'Rightside','FontSize',12,'Color',color);
    text(h.axMain,datasize(1)/2,datasize(2),max(h.proMat(:))*2/3,'Leftside','FontSize',12,'Color',color);
elseif strcmp(cT,'3D')||strcmp(cT,'Bars')
    text(h.axMain,0, datasize(2)/2,max(h.proMat(:))*2/3,'Rightside','FontSize',12,'Color',color);
    text(h.axMain,datasize(1), datasize(2)/2,max(h.proMat(:))*2/3,'Leftside','FontSize',12,'Color',color);
    text(h.axMain,datasize(1)/2,0,max(h.proMat(:))*2/3,'Tail','FontSize',12,'Color',color);
    text(h.axMain,datasize(1)/2,datasize(2),max(h.proMat(:))*2/3,'Head','FontSize',12,'Color',color);
end
...
if get(h.tbPoints_Pro,'Value')
    imagesc(sum(h.coordMat,3)');
    myColorMap = jet(256);
    myColorMap(1,:) = 1;
    colormap(h.axMain,myColorMap);
    contour(h.proMat'/max(h.proMat(:))*max(max(sum(h.coordMat,3))),'PickableParts','none');
else
    imagesc(h.proMat'),colormap(h.axMain,'jet');
    axis tight manual
end
if get(h.cbLabelAxis,'Value')
    if get(h.rbSamples,'Value')
        labelAxis(h,[h.samples,h.samples],'Pro','magenta');
    elseif get(h.rbDatasize,'Value')
        labelAxis(h,size(h.proMat),'Pro');
    end
end

if get(h.tbColorbar,'Value')
    colorbar;
end
hold off;
...
if get(h.cbLabelAxis_Slice,'Value')
    if get(h.rbSamples,'Value')
        labelAxis(h,[h.samples,h.samples],'Slice','magenta');
    elseif get(h.rbDatasize,'Value')
        labelAxis(h,size(h.proMat),'Slice');
    end
end

if get(h.tbColorbar_Slice,'Value')
    colorbar;
end
...
if get(h.cbLabelAxis_3D,'Value')
    if get(h.rbSamples,'Value')
        labelAxis(h,[h.samples,h.samples],'3D','magenta');
    elseif get(h.rbDatasize,'Value')
        labelAxis(h,size(h.proMat),'3D');
    end
end
...
% Transparancy
transArray = [1, 0.5, 0.4 , 0.3 , 0.2 , 0.1];
...
% We want that the upper 10% should be red.
maxD = maxD*0.9;
...
for i = 1:numOfLayers
    ...
    isonormals(data,p), daspect([1 1 1]), axis tight manual;
end
camlight(-80,-10), lighting gouraud;
...
if get(h.tb2DOnePlot,'Value')
    ...
    maxBarValue = max([max(barMat1(:)),max(barMat1(:)),max(barMat3(:))]);
    % Set limits
    if get(h.rbSamples,'Value')
        set([h.subax1,h.subax2,h.subax3],'XLim',[1, h.samples]);
        set([h.subax1,h.subax2,h.subax3],'YLim',[1, maxBarValue]);
    elseif get(h.rbDatasize,'Value')
...
if get(h.cbLabelAxis_Bars,'Value')
    if get(h.rbSamples,'Value')
        labelAxis(h,[h.samples,h.samples],'Bars','magenta');
    elseif get(h.rbDatasize,'Value')
        labelAxis(h,size(proMat),'Bars');
    end
end
...
clickTab(hObject);
h.currentTab = '3D';
set(h.txSlicer,'Visible','On',...
    'Parent',h.paVis_3D,...
    'String',['Layer ', num2str(1),' to ', num2str(get(h.slCloudLayer,'Value'))]);
guidata(hObject,h);
...
draw3DMap(h);
set(h.txSlicer,'String',['Layer ', num2str(1),' to ', num2str(get(h.slCloudLayer,'Value'))]);
...
if get(h.cbLabelAxis_Bars,'Value')
    if get(h.rbSamples,'Value')
        labelAxis(h,[h.samples,h.samples],'Bars','magenta');
    elseif get(h.rbDatasize,'Value')
        labelAxis(h,size(h.proMat),'Bars');
    end
else
    axes(h.axMain),drawBars3D(h);
end
...
% --- Executes on button press in tbColorbar.
function tbColorbar_Callback(hObject, eventdata, handles)
% hObject    handle to tbColorbar (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of tbColorbar
...
% --- Executes on button press in tbColorbar_Slice.
function tbColorbar_Slice_Callback(hObject, eventdata, handles)
% hObject    handle to tbColorbar_Slice (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of tbColorbar_Slice
