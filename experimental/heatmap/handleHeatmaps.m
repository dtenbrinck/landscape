function [  ] = handleHeatmaps( accumulator, numOfAllCells,numberOfResults, p, option, filename )
...
if nargin == 5
    filename = ''; % specify empty string in default case
else
    filename = [filename '_']; % add underscore to filename
end
...
if option.heatmaps.process == 1
    ...
    if option.heatmaps.saveHMmat == 1
        mat_name = [p.resultsPath ,'/heatmaps/',filename,'Heatmap_Structure.mat'];
        save(mat_name,'HMS');
    end
    
    % Save accumulator
    if option.heatmaps.saveAccumulator == 1
        mat_name = [p.resultsPath ,'/heatmaps/',filename,'Accumulator.mat'];
    ...
    for i=1:size(option.heatmaps.types,2)
        ...
        if strcmp(option.heatmaps.scaled,'true')||strcmp(option.heatmaps.scaled,'both')
            ...
            if option.heatmaps.save == 1
                ...
                for j=1:size(option.heatmaps.saveas,2)
                    fig_filename = [p.resultsPath ,'/heatmaps/',filename,currentType,'_Heatmaps'];
        ...
        if strcmp(option.heatmaps.scaled,'false')||strcmp(option.heatmaps.scaled,'both')
            ...
            if option.heatmaps.save == 1
                ...
                for j=1:size(option.heatmaps.saveas,2)
                    fig_filename = [p.resultsPath ,'/heatmaps/',filename,currentType,'_Heatmaps(unscaled)'];
                    saveas(f,fig_filename,option.heatmaps.saveas{j});
                end
            end
        end
