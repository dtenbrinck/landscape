function experimentSets = checkExperimentChannelsForBFPandGFPdata( fileNames )
...
for i = nb_experiments
    ...
    if any(contains(fileNames(indices),'BFP'))
        index = strfind(fileNames(indices),'BFP');
    elseif any(contains(fileNames(indices),'DAPI'))
